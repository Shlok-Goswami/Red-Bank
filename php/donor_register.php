...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($_POST as $key => $value) {
        if (!isset($_POST[$key])) {
            header("location: ../donateBlood.php?error=$key was not found");
            die();
        }
    }
    ...
    // Fetch the donor's ID based on their email 
    ...
    if ($checkRegisteredResult->num_rows > 0) {
        header("location: ../become_donor.php?error=Already registered at this camp");
        die();
    }
